...
double sensor_value(double x_val, double y_val, double z_val) 
{
...
void main(void)
{
	...
	while (1) {
		...
		if (sensor_value_to_double(&temp1) > 24.5 && sensor_value_to_double(&temp1) < 30.0 && sensor_value_to_double(&press) > 90.0 && sensor_value_to_double(&press) < 110.0) 
		{
			/* lsm6dsl accel */
		double gravity = sensor_value(sensor_value_to_double(&accel1[0]), sensor_value_to_double(&accel1[1]), sensor_value_to_double(&accel1[2]));
		
		if(gravity > 9.804 && gravity < 9.834)
		{
			printf("Gravity acceleration measured somewhere between Spain and Norway (by the value of latitudes).\nLSM6DSL: Accel (m.s-2): x: %.1f, y: %.1f, z: %.1f -> g = %.3f\n",
		...
		/* lsm6dsl gyro */
		double omega = sensor_value(sensor_value_to_double(&gyro[0]), sensor_value_to_double(&gyro[1]), sensor_value_to_double(&gyro[2]));
		printf("LSM6DSL: Gyro (dps): x: %.3f, y: %.3f, z: %.3f -> rotational acceleration = %.3f\n",
		...
		/* lsm303agr accel */
		double linear_type2 = sensor_value(sensor_value_to_double(&accel2[0]), sensor_value_to_double(&accel2[1]), sensor_value_to_double(&accel2[2]));
		printf("LSM303AGR: Accel (m.s-2): x: %.1f, y: %.1f, z: %.1f -> linear acceleration = %.3f\n",
		...
		/* lsm303agr magn */
		double magnetic_field = sensor_value(sensor_value_to_double(&magn[0]), sensor_value_to_double(&magn[1]), sensor_value_to_double(&magn[2]));
		printf("LSM303AGR: Magn (gauss): x: %.3f, y: %.3f, z: %.3f -> magnetic field = %.3f\n",
		...
		{
			printf("External conditions and parameters are not satisfactory for recording the data (linear and rotational acceleration, value of magnetic field (magnetic dipole moment) from the sensor!\n");
		}  
